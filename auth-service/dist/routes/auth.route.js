...
export default async function authRoutes(fastify) {
    ...
    fastify.post('/login', async (request, reply) => {
        const { email, password, posteId } = request.body;
        if (!email || !password) {
        ...
        try {
            ...
            const userRes = await client.query(`SELECT id, email, password_hash, nom, prenom, telephone, site_id, actif 
         FROM users WHERE email = $1`, [email.toLowerCase().trim()]);
            ...
            const rolesRes = await client.query(`SELECT r.id, r.nom, r.permissions 
         FROM roles r 
         ...
         WHERE ur.user_id = $1`, [user.id]);
            ...
            await client.query(`INSERT INTO sessions (user_id, refresh_token, expires_at, poste_id, ip, user_agent)
         VALUES ($1, $2, $3, $4, $5, $6)`, [user.id, refreshTokenHash, expiresAt, posteId || null, request.ip, request.headers['user-agent'] || '']);
    ...
    fastify.post('/refresh', async (request, reply) => {
        ...
        try {
            ...
            const sessionRes = await client.query(`SELECT s.id, s.user_id, s.refresh_token, u.email, u.site_id, u.actif
         FROM sessions s
         ...
         FROM roles r 
         JOIN user_roles ur ON ur.role_id = r.id 
         WHERE ur.user_id = $1`, [matchedSession.user_id]);
            const roles = rolesRes.rows;
            const primaryRole = roles[0]?.nom || 'médecin';
            const allPermissions = Array.from(new Set(roles.flatMap((r) => r.permissions || [])));
    ...
    fastify.get('/me', async (request, reply) => {
        ...
        try {
            ...
            const client = await fastify.pg.connect();
            try {
                const userRes = await client.query(`SELECT id, email, nom, prenom, telephone, site_id, actif, derniere_connexion 
           ...
           FROM roles r 
           JOIN user_roles ur ON ur.role_id = r.id 
           WHERE ur.user_id = $1`, [payload.sub]);
                return reply.send({
                    user: userRes.rows[0],
                    roles: rolesRes.rows,
    ...
    fastify.post('/pairing/code', async (request, reply) => {
        ...
        const client = await fastify.pg.connect();
        try {
            const res = await client.query(`INSERT INTO pairing_codes (code, site_id, expires_at)
         VALUES ($1, $2, $3) RETURNING id, code, expires_at`, [code, siteId || '00000000-0000-0000-0000-000000000000', expiresAt]);
            return reply.send({

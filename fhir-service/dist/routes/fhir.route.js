export default async function fhirRoutes(fastify) {
    ...
    fastify.get('/:resourceType', async (request, reply) => {
        ...
        try {
            const countRes = await client.query(`SELECT COUNT(*) AS total FROM fhir_resources WHERE ${whereClause}`, params);
            const total = parseInt(countRes.rows[0].total, 10);
            const dataRes = await client.query(`SELECT fhir_id, content, last_updated FROM fhir_resources 
         WHERE ${whereClause} 
    ...
    fastify.get('/:resourceType/:fhirId', async (request, reply) => {
        const { resourceType, fhirId } = request.params;
        const client = await fastify.pg.connect();
        try {
            const res = await client.query(`SELECT content, last_updated FROM fhir_resources WHERE resource_type = $1 AND fhir_id = $2`, [resourceType, fhirId]);
    ...
    fastify.post('/:resourceType', async (request, reply) => {
        const { resourceType } = request.params;
        const body = request.body;
        // Extraire le clientMutationId (body ou header HTTP X-Client-Mutation-Id)
        const clientMutationId = body.clientMutationId ||
            request.headers['x-client-mutation-id'] ||
            null;
        const client = await fastify.pg.connect();
        try {
            // 4.2 Idempotence côté serveur : si déjà reçu, renvoyer la ressource existante
            if (clientMutationId) {
                const existing = await client.query(`SELECT content FROM fhir_resources WHERE client_mutation_id = $1`, [clientMutationId]);
            ...
            const content = { ...body, id: fhirId, resourceType };
            ...
            await client.query(`INSERT INTO fhir_resources (resource_type, fhir_id, content, subject_id, encounter_id, status, client_mutation_id)
         ...
         ON CONFLICT (resource_type, fhir_id) DO UPDATE SET
           ...
           last_updated = NOW()`, [resourceType, fhirId, JSON.stringify(content), subjectId, encounterId, status, clientMutationId]);
    ...
    fastify.put('/:resourceType/:fhirId', async (request, reply) => {
        const { resourceType, fhirId } = request.params;
        const body = request.body;
        const ifMatchHeader = request.headers['if-match']; // e.g., 'W/"1"' or '1'
        const client = await fastify.pg.connect();
        try {
            const existingRes = await client.query(`SELECT content FROM fhir_resources WHERE resource_type = $1 AND fhir_id = $2`, [resourceType, fhirId]);
            ...
            const encounterId = body.encounter?.reference?.replace(/^Encounter\//, '') || body.encounterId || null;
            const status = body.status || null;
            await client.query(`UPDATE fhir_resources 
         SET content = $1, subject_id = $2, encounter_id = $3, status = $4, last_updated = NOW()
         WHERE resource_type = $5 AND fhir_id = $6`, [JSON.stringify(updatedContent), subjectId, encounterId, status, resourceType, fhirId]);
    ...
    fastify.get('/sync/bootstrap', async (request, reply) => {
        // Role récupéré du JWT ou header pour le périmètre de sync
        const userRole = request.user?.role || 'médecin';
        ...
        const client = await fastify.pg.connect();
        try {
            const res = await client.query(`SELECT resource_type, fhir_id, content, last_updated 
         FROM fhir_resources 
         WHERE resource_type = ANY($1)
         ORDER BY last_updated DESC 
         LIMIT 500`, [targetResources]);
...
//# sourceMappingURL=fhir.route.js.map
...
// ─── Références FHIR (types légers — les ressources complètes vivent dans fhir-service) ─

export interface FhirReference {
  reference: string;           // ex: "ServiceRequest/uuid"
  display?: string;
}
...
export interface LaboServiceRequest {
  ...
  code?: FhirCodeableConcept;  // type d'examen demandé
...
export interface LaboObservation {
  ...
  valueString?: string;
  referenceRange?: Array<{ low?: number; high?: number; unit?: string; text?: string }>;
...
export interface LaboDiagnosticReport {
  ...
  issued?: string;
  conclusion?: string;
  observationIds: string[];    // IDs des Observation liées
}

// ─── TestCatalog (Custom — référentiel local du laboratoire) ──────────────

export interface ValeurReference {
  sexe?: 'M' | 'F' | 'tous';
  âgeMin?: number;             // en années
  âgeMax?: number;
  min?: number;
  max?: number;
  unité: string;
...
export interface TestCatalog {
  id: string;                  // UUID
  nomExamen: string;
  code: string;                // code LOINC ou code local
  système?: string;            // ex: "http://loinc.org"
  catégorie?: string;          // ex: "hématologie", "biochimie", "microbiologie"
  délaiEstimé?: number;        // en heures
  prix?: number;               // en unité monétaire locale
